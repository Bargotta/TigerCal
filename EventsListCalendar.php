...
Calendar for the Events List Page
...
class EventsListCalendar
{
    ...
    {        
        ...
        $this->todayDate = date('Y')."-".date('m')."-".date('d');
    }
    ...
    private function createForm() {
        ...
        if (isset($_POST['calendarDate'])) {
            ...
            if (!empty(trim($date)))
                header('location: '.$this->hrefNav.'?date='.$date.'#events');
        }

        $contents = '<div class="col-sm-4">'.
                        '<div class="col-sm-9">'.
                            '<form action="#events" class="form text-center" role="form" method="post">'.
                                '<input type="date" name="calendarDate" />'.
                                '<input type="submit" value="submit" name="submitCalendarDate" />'.
                            '</form>'.
                        '</div>'.
                        '<div class="col-sm-3">'.
                            '<a href="'.$this->hrefNav.'?date='.$this->todayDate.'#events" onClick="history.go(0)"><button>Today</button></a>'.
                        '</div>';
        return $contents;
    }
        ...
        return
            '<ul class="pager">'.
                '<li class="previous"><a href="'.$this->hrefNav.'?date='.sprintf('%02d',$preYear).
                    '-'.$preMonth.'-0#events"><span class="glyphicon glyphicon-chevron-left"></span></a></li>'.
                ...
                '<li class="next"><a href="'.$this->hrefNav.'?date='.sprintf("%02d", $nextYear).'-'.
                    $nextMonth.'-0#events"><span class="glyphicon glyphicon-chevron-right"></span></a></li>'.
            '</ul>';
    }
    ...
    private function createTag($formattedDay, $isCurrDate, $digits, $addSelected) {
        ...
        $result = mysqli_query($conn, $query) or die("Could not search!".mysqli_error($conn));
